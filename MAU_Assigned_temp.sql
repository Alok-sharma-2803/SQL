create table analytics_reporting.mau_assigned_temp as (
...
traffic as (select month, userid, gaid, appname, lead_score
            from (select date_trunc('mon', c.timestampist) as month,
                	...
                	where date(c.timestampist) between '2021-03-01' and '2021-03-21')
    	    where rn = 1),
install as (select * from (select  date_trunc('mon', trunc(timestamp 'epoch'+(timestamp+19800000)/1000*interval '1 second')) as month,
			...
			where date(trunc(timestamp 'epoch'+(timestamp+19800000)/1000*interval '1 second')) between '2021-03-01' and '2021-03-21')
			where rn =1),
servicable as (select date(record_date) as date,
                ...
                AND record_date::date between '2021-03-01' and '2021-03-21'
                AND lead_score >= 6
                group by 1,2,3,4),
assigned as (select distinct prospectid
...
source as (select cg.month,
        		...
        		case when new_signup = 1 and i.month = cg.month then 'newsignup_newinstall'
...
select case when appname in ('GROWTHAPP', 'DEXAPP') then 'App' else 'Web' end as "App/Web",
        ...
        grade,
        case when s.lead_score between 0 and 5 then '0 - 5'
                when s.lead_score > 5 then '5+'
                else 'Others' end as ls_bucket,
        ...
        count(distinct case when a.userid is not null then pd.prospectid end) as assigned
from source s
